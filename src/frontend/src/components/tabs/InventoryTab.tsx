...
import { Package, Paperclip, Pencil, Plus, Search, User } from "lucide-react";
...
export default function InventoryTab() {
  ...
  const [attachmentsItem, setAttachmentsItem] = useState<InventoryItem | null>(
    null,
  );
  ...
  const totalValue = filtered.reduce(
    (sum, item) => sum + item.costPerUnit * Number(item.quantity),
    0,
  );
  ...
  const handleOpenAttachments = (item: InventoryItem) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Manage farm inventory and stock levels
          </p>
        </div>
        <Dialog
          open={open}
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value={ItemType.fertilizer}>
                      Fertilizer
                    </SelectItem>
                    <SelectItem value={ItemType.pesticide}>
                      Pesticide
                    </SelectItem>
                    <SelectItem value={ItemType.equipment}>
                      Equipment
                    </SelectItem>
                  </SelectContent>
                </Select>
      ...
      <div className="bg-secondary/30 border border-secondary/40 rounded-xl p-4 flex items-center gap-4">
        ...
        <div>
          <p className="text-sm text-muted-foreground">
            Total Inventory Value (filtered)
          </p>
              ...
              {isLoading ? (
                ...
                filtered.map((item) => (
                  ...
                  >
                    ...
                    <TableCell className="text-right">
                      {formatNaira(item.costPerUnit)}
                    </TableCell>
                    <TableCell className="text-right font-semibold">
                      {formatNaira(item.costPerUnit * Number(item.quantity))}
                    </TableCell>
      ...
      {attachmentsItem && (
        <AttachmentsDialog
          ...
          onOpenChange={(v) => {
            setAttachmentsOpen(v);
            if (!v) setAttachmentsItem(null);
          }}
          inventoryItemId={attachmentsItem.id}
          inventoryItemName={attachmentsItem.name}
        />
      )}
    </div>
  );
